...
Choose **Delete** on a saved scene and confirm to remove it. Recordings shared by another saved scene stay available; orphaned generated WAVs are removed. Deleting a saved scene leaves current playback alone, so file cleanup waits until all open sessions are closed. Imported recordings are kept. Deleted scenes stay deleted through resume and restart; preparing the description again creates a new saved entry and can still reuse recordings shared with other scenes.

The library migrates automatically to SQLite v4 on server startup and includes still-retained prepared sessions. Repeated identical descriptions/settings share one saved entry. Sessions deleted before the library feature cannot be reconstructed. The migration conservatively preserves potentially shared legacy event recordings because older versions did not track every reuse. Interrupted file cleanup retries after the last session closes, another deletion, or a restart without open sessions. The uncached `GET /api/scenes?q=...&offset=0` returns at most twelve entries; reading it never creates listeners, starts inference or renews playback activity. `DELETE /api/scenes/:id` removes a saved recipe and queues orphan cleanup. Restart an older running server after updating; for the installed PWA, close and reopen the app to pick up the new shell once available.
