...
**Current state: Phase 6 software implemented; final acceptance in progress.** Ollama parses a setting; Stable Audio 3 Medium (MLX on this Mac) creates four contextual 90-second beds and optional sparse events. Validated assets feed continuous AAC/HLS playback and persist in SQLite/filesystem storage for reuse. Existing Phase 3 sessions retain their procedural beds; create a new prompted scene to hear generated audio. The user-reported overnight listening check passed on 2026-09-24, with about 63 minutes of playback before the sleep buds automatically paused. An eight-hour laptop soak also passed with 961 complete active samples and bounded resources for the layered ambience/effects scene. Representative listening checks and eight-hour physical-iPhone acceptance remain open; PWA diagnostic recording is planned to remove the separate terminal setup. See [ACCEPTANCE.md](ACCEPTANCE.md#eight-hour-laptop-soak--2026-09-24) for scope and measurements. [SPEC.md](SPEC.md) defines v0.1; [PLAN.md](PLAN.md) tracks implementation and acceptance evidence separately.
