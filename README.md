...
**Regenerate scene** asks for confirmation, clears that description using the same deletion rules, and immediately starts fresh preparation with the selected saved entry’s original prompt, Simple/Layered mode and Sleep setting. It closes matching sessions, preserves genuinely shared/imported recordings, and excludes old recordings from the replacement. You can regenerate the scene currently on this page; stop a different current scene first. If unrelated sessions fill the server, close one before regenerating.
...
Open **Playback diagnostics** using the footer link. It has its own `/diagnostics` page; navigating there and back within the app preserves playback. Choose **5-minute check** or **8-hour recording**, enter the device/OS/audio-output details, and tap **Start recording**. The current session is selected automatically when following that link; you can also choose another open session. Opening the route directly only reads status and never joins or starts playback. Wait for the complete-sample count to increase before locking the device. Samples are collected on the server every five seconds for the check, or every thirty seconds for the overnight run. No separate recorder terminal or session ID is needed.
