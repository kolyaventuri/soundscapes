...
- For audio or session lifecycle changes, also run `pnpm audio:smoke` and `pnpm audio:integration` with real FFmpeg; for ambience changes, add `pnpm audio:mixer` and `pnpm audio:phase2`. For planner/event changes, also run `pnpm audio:phase3`; run `pnpm planner:smoke` when changing the real model or its prompts, using the configured local runtime. Status/playlist polling must not keep a listener active; explicit pause must remain idle despite stale requests.
- Phase 3 uses native Ollama/Qwen3 8B with cloud disabled, constrained JSON, bounded cancellable requests, and unload-after-request. Use reviewed local WAVs; retain explicit model versus physical listening evidence. Scene-specific audio generation is Phase 4.
