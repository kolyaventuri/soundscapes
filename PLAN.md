...
- [ ] Polish initial generation feedback: show live stages (understanding, waiting/loading, generating, preparing playback), completed beds and meaningful within-stage progress where available; make the transition to playable audio clear without exposing model internals.
- [ ] Estimate time until playback from measured timings for the selected model/backend and remaining work, including loading, queueing, reuse, validation and buffering. Show an approximate range that updates as work completes; show an honest estimating/indeterminate state when evidence is insufficient, rather than a fabricated countdown.
- [ ] Verify preparation feedback on mobile for cold/warm starts, reused assets, slow or stalled work, cancellation, errors and reconnects. Progress must reflect server state, avoid premature 100%/ready claims, and never keep a paused listener active. Deferred UX polish; current stage text remains usable meanwhile.
