...
- [ ] Verify preparation feedback on mobile for cold/warm starts, reused assets, slow or stalled work, cancellation, errors and reconnects. Progress must reflect server state, avoid premature 100%/ready claims, and never keep a paused listener active. Automated timing/protocol/lifecycle coverage and desktop mobile-viewport checks are recorded below; physical iPhone verification remains open.
...
- [ ] Add a scene controller with validated prompt-derived per-layer policies: continuity, weighted pools, gap ranges, crossfades, variability, repetition cooldowns and overlap limits. Support continuous cafe music versus live-band gaps, continuous ambience/crowds and sparse short effects; avoid per-transition LLM calls.
- [ ] Give layers independent clip boundaries and seeded random sequences on the shared playback clock; persist scheduling state across pause/restart. Verify music continues unchanged across ambience fades, adding effects does not reshuffle music, and render/HLS boundaries never dictate content transitions.
- [ ] Bound and prune each layer's future timeline; prepare minimum usable pools and expand optional libraries in the background within inference/idle limits. Test deterministic scheduling with fixed pools/policies, overlap/headroom limits, gaps, cancellation and restart recovery.
