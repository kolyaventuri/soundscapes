...
- [ ] Confirm lock-screen resume → playback.
- [x] Disconnect Wi-Fi without sending pause; confirm the listener expires after 90 seconds (user-reported).
- [ ] Confirm after expiry that the session is idle, `rendering: false`, and `producerPid: null` with no other listeners; then verify reconnect.
